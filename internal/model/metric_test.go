...
func TestFromURL(t *testing.T) {
	tests := []struct {
		...
		{
			name:    "valid short URL for empty gauge",
			url:     "/update/gauge/m1",
			wantErr: false,
			want:    Metric{Name: "m1", Type: MetricTypeGauge},
		},
		{
			name:    "valid short URL for empty counter",
			url:     "/update/counter/m1",
			wantErr: false,
			want:    Metric{Name: "m1", Type: MetricTypeCounter},
		},
		{
			name:    "valid URL for gauge -- float string",
			url:     "/update/gauge/m1/10.0",
			want:    Metric{Name: "m1", Type: MetricTypeGauge, Value: func(f float64) *float64 { return &f }(10)},
			wantErr: false,
		},
		{
			name:    "valid URL for gauge -- int string",
			url:     "/update/gauge/m1/10",
			want:    Metric{Name: "m1", Type: MetricTypeGauge, Value: func(f float64) *float64 { return &f }(10)},
			wantErr: false,
		},
		{
			name:    "valid URL for counter -- int string",
			url:     "/update/counter/m1/10",
			wantErr: false,
			want:    Metric{Name: "m1", Type: MetricTypeCounter, Delta: func(f int64) *int64 { return &f }(10)},
		},
		{
			name:    "invalid URL for counter -- float string",
			url:     "/update/counter/m1/10.0",
			wantErr: true,
		},
		{
			name:    "invalid URL -- no name, no value #1",
			url:     "/update/gauge/",
			wantErr: true,
		},
		{
			name:    "invalid URL -- no name, no value #2",
			url:     "/update/gauge",
			wantErr: true,
		},
		{
			name:    "invalid short URL -- invalid type",
			url:     "/update/wrong/m1",
			wantErr: true,
		},
		{
			name:    "invalid short URL -- invalid name for gauge",
			url:     "/update/gauge/1",
			wantErr: true,
		},
		{
			name:    "invalid short URL -- invalid name for counter",
			url:     "/update/counter/1",
			wantErr: true,
		},
		{
			name:    "invalid URL -- no value",
			url:     "/update/gauge/m1/",
			wantErr: true,
		},
		{
			name:    "invalid URL for gauge -- string value",
			url:     "/update/gauge/m1/value",
			wantErr: true,
		},
		{
			name:    "invalid URL for counter -- string value",
			url:     "/update/counter/m1/value",
			wantErr: true,
		},
	}
